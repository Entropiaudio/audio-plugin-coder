...
#include "API/HttpHelper.h"
  ...
  #if ! __has_include("IntegrityCheck.generated.h")
    #error "IntegrityCheck.generated.h is missing! This file should be generated by the `moonbase_JUCEClient/PreBuild.sh` script, make sure to run that before building."
  #endif
#endif
  
#include "IntegrityCheck.generated.h"
...
#include "API/APIImpl.h"
...
#if JUCE_WINDOWS
    #pragma warning(push)
    #pragma warning(disable:4505) // unreferenced local function has been removed, disabled so tests
    //the following two double up on SupressMSVCWarnings.h, but are guarded by the push/pop here
    #pragma warning(disable: 4100) // unreferenced formal parameter has been removed
    #pragma warning(disable: 4702) // unreachable code has been removed
#endif
    ...
    static inline bool RunningOnRosetta ()
    {
        int rosetta = 0;
...
static inline char GetMachinePlatformPrefix()
{
...
static inline const juce::String GetSHA256EncodedIDString (const juce::String& input, const APIImpl& api)
{
...
OBF_BEGIN
    ...
    const auto dataToHash = platform + api.getMachineHashSalt() + input;
...
static inline const juce::String GetMachineHash (const APIImpl& api)
{
OBF_BEGIN
    ...
    #else
        if (api.initializer.machineIdCallback != nullptr)
        {
            const auto id = api.initializer.machineIdCallback();
            // If you're providing a custom machine ID, it must at the very least not be empty
            jassert (id.isNotEmpty());
            return id;
        }

        return GetSHA256EncodedIDString (juce::SystemStats::getUniqueDeviceID (), api);
    #endif
OBF_END
...
static inline const std::pair<juce::String, juce::String> GetMachineNameAndSignature (const APIImpl& api)
{
OBF_BEGIN
    return { juce::SystemStats::getComputerName (), GetMachineHash (api) };
OBF_END
...
OBF_BEGIN
    ...
    auto meta64         { lines[metaLine] };  meta64 = meta64.upToFirstOccurrenceOf ("\r", false, false).trim();
...
OBF_END
    ...
    static inline const juce::String PropertiesId { "properties" };
...
static inline bool GetLicenseIsOfflineActivatedFromLicense (const APIImpl& api)
{
OBF_BEGIN
...
static inline bool GetTrialStateFromLicense (const APIImpl& api)
{
OBF_BEGIN
...
OBF_BEGIN
    ...
    if (str.isNotEmpty ())
    {
        ...
        const auto zChar = static_cast<juce::juce_wchar>('Z');
...
static inline bool HasNonTextChildElements (const juce::XmlElement* element)
{
...
static inline void FlattenXmlProperties (const juce::XmlElement* element, const juce::String& prefix, juce::StringPairArray& result)
{
    for (auto* child = element->getFirstChildElement(); child != nullptr; child = child->getNextElement())
    {
        if (child->isTextElement())
        ...
        const auto key = prefix.isEmpty() ? child->getTagName() : prefix + "." + child->getTagName();

        if (HasNonTextChildElements (child))
            FlattenXmlProperties (child, key, result);
        else
            result.set (key, child->getAllSubText());
    }
}

static inline const juce::StringPairArray GetPropertiesFromLicense (const APIImpl& api, const juce::String& groupName)
{
OBF_BEGIN
    auto localLicenseFile { api.getLicenseFile () };
    ...
    IF (root != nullptr)
    {
        auto propertiesElement = root->getChildByName (PropertiesId);
        IF (propertiesElement != nullptr)
        {
            auto groupElement = propertiesElement->getChildByName (groupName);
            IF (groupElement != nullptr)
            {
                juce::StringPairArray props;
                FlattenXmlProperties (groupElement, "", props);
                RETURN(props);
            }
            ENDIF
        }
...
OBF_BEGIN
    ...
    const auto meta64    { lines[metaLine].upToFirstOccurrenceOf ("\r", false, false).trim() };  
    const auto sig64     { lines[sigLine].trim() };
    ...
    const auto keyIntegrity = (int)VerifyMoonbaseKeyIntegrity (rsaModulus);
    ...
    const auto localMachineSig   { GetMachineNameAndSignature(api).second };
...
static inline juce::URL WithAnalyticsParameters (juce::URL url, const APIImpl& api)
{
    ...
    for (const auto& key : defaultAnalytics.getAllKeys ())
        url = url.withParameter (key, defaultAnalytics[key]);

    for (const auto& key : extendedAnalytics.getAllKeys ())
        url = url.withParameter ("meta[" + key + "]", extendedAnalytics[key]);

    for (const auto& key : userAnalytics.getAllKeys ())
        url = url.withParameter ("meta[" + key + "]", userAnalytics[key]);
