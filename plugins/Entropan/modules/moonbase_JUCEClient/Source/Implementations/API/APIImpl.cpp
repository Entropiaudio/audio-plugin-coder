...
#include "ValidationCoordinator.cpp"
#endif

#if defined(__has_include)
    #if ! __has_include("../../ConfigDetails.generated.h")
        #error "ConfigDetails.generated.h is missing! This file should be generated by the `moonbase_JUCEClient/PreBuild.sh` script, make sure to run that before building."
    #endif
#endif
#include "../../ConfigDetails.generated.h"
...
//==============================================================================
//==============================================================================
APIImpl::APIImpl (const APIInitializer& init, API& apiRef)
:
initializer (init),
api (apiRef)
{
    ...
    if (init.customMachineHashSalt.isNotEmpty ())
        setCustomMachineHashSalt (init.customMachineHashSalt);
        
    activationWatcher = std::make_unique <ActivationWatcher>       (*this);

#ifndef MB_CATCH2_TESTING
...
APIImpl::~APIImpl ()
{
    activationWatcher.reset ();
    cancelPendingUpdate ();
    autoActivationRequestCallback = nullptr;
    getAnalyticsCallback          = nullptr;
    activationStateChangedCallbacks.clearQuick ();

    signalDestroyer.reset ();        // PeriodicSignalDestroyer (offline validation thread)
    onlineValidator.reset ();        // owns its own HttpHelper thread
    autoActivationProcessor.reset ();
    httpHelpers.clear (true);        // joins any in-flight network threads

#if INCLUDE_MOONBASE_UI
    currentUi = nullptr;
    machineFileChooser.reset ();
#endif
}

const juce::String APIImpl::getProductId () const
{
OBF_BEGIN
    return Moonbase::ConfigDetails::ProductId::create ();
OBF_END
    return {};
}
...
{
OBF_BEGIN
    const auto configLocation = Moonbase::ConfigDetails::LicenseFileLocation::create ();
    auto licenseFile = juce::File::getSpecialLocation (juce::File::userApplicationDataDirectory).getChildFile (configLocation);
    #if JUCE_MAC
        if (RunningOnRosetta ())
        {
...
const juce::URL APIImpl::getBaseUrl () const
{
OBF_BEGIN
    return juce::URL (Moonbase::ConfigDetails::BaseUrl::create ());
OBF_END
    return {};
}

const juce::URL APIImpl::getOfflineUrl () const
{
OBF_BEGIN
    return juce::URL (Moonbase::ConfigDetails::OfflineUrl::create ());
OBF_END
    return {};
}

const juce::String APIImpl::getPublicKey () const
{
OBF_BEGIN
    return Moonbase::ConfigDetails::PublicKey::create ();
OBF_END
    return {};
}
...
const juce::StringPairArray APIImpl::getProductProperties () const
{
OBF_BEGIN
    return GetPropertiesFromLicense (*this, "product");
OBF_END
    return {};
}

const juce::StringPairArray APIImpl::getCustomerProperties () const
{
OBF_BEGIN
    return GetPropertiesFromLicense (*this, "customer");
OBF_END
    return {};
}

const juce::StringPairArray APIImpl::getLicenseProperties () const
{
OBF_BEGIN
    return GetPropertiesFromLicense (*this, "license");
OBF_END
    return {};
}

const juce::StringPairArray APIImpl::getTrialProperties () const
{
OBF_BEGIN
    return GetPropertiesFromLicense (*this, "trial");
OBF_END
    return {};
}
...
void APIImpl::requestAutoActivation (const AutoActivationRequestCallback& callback)
{
    ...
    OBF_BEGIN
        ...
        static const auto machineNameAndSig { GetMachineNameAndSignature (*this) };

        juce::DynamicObject::Ptr postObj (new juce::DynamicObject());
        postObj->setProperty (machineNameId, machineNameAndSig.first);
        postObj->setProperty (machineSignatureId, machineNameAndSig.second);
        const auto postBody = juce::JSON::toString (juce::var (postObj.get()));

        const auto url = WithAnalyticsParameters (
                            getBaseUrl ()
                                .getChildURL ("/api/client/activations/" + getProductId () + "/request")
                                .withParameter ("format", "Legacy"),
                            *this)
                        .withPOSTData (postBody);
...
void APIImpl::requestOnlineActivation (const juce::String& email, const juce::String& password)
{
    ...
    OBF_BEGIN
        ...
        postObj->setProperty (machineNameId, machineNameAndSig.first);
        postObj->setProperty (machineSignatureId, machineNameAndSig.second);
        postObj->setProperty ("username", email);
        postObj->setProperty ("password", password);
        const auto postBody = juce::JSON::toString (juce::var (postObj.get()));

        const auto url = WithAnalyticsParameters (
                            getBaseUrl ()
                                .getChildURL ("/public/" + getProductId () + "/activate")
                                .withParameter ("format", "Legacy"),
                            *this)
                        .withPOSTData (postBody);
...
bool APIImpl::isUpdateAvailable () const
{
OBF_BEGIN
    ...
    const auto parseVersionToInt = [](const juce::String& version) -> int
    {
        ...
        for (int i = 0; i < parts.size(); ++i)
        {
            ...
            result += part * static_cast<int>(std::pow(1000, parts.size() - i - 1));
        }
...
int APIImpl::getGracePeriodDays () const
{
OBF_BEGIN
...
bool APIImpl::transmitsAnalytics () const
{
OBF_BEGIN
...
bool APIImpl::includesExtendedDefaultAnalytics () const
{
OBF_BEGIN
...
void APIImpl::generateMachineFile (const MachineFileCallback& callback, juce::File saveLocation)
{
    ...
    OBF_BEGIN
        ...
        {
            const auto machineNameAndSig { GetMachineNameAndSignature (*this) };
            juce::String content;
...
void APIImpl::setCustomMachineHashSalt (const juce::String& salt)
{
    machineHashSalt = salt;
}

juce:: String APIImpl::getMachineHashSalt () const
{
    return machineHashSalt;
}
