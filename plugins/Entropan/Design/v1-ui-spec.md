...
```
┌────────────────────────────────────────────────────────────────┐
│ HEADER (56px): title+eyebrow | ↶ CHAOS ↷  preset ▾ | ENTROPY ◔ seed │
├────────────────────────────────────────────────────────────────┤
...
│  BAND [freq width depth + ON]  MOD [mode chips; rate/inertia/   │
...
- **Bells:** each enabled band = bell curve; height = `depth` (lift), center X = `freq`, width = `width` oct. Fill = band color @ 18% alpha, stroke = band color, handle dot at apex.
- Live pan trace: thin horizontal indicator inside each bell showing current mod pan position (animates; preview fakes it).
- Gestures:
  - Drag handle: X = freq, Y = depth.
...
- Right: **ENTROPY** master — 44px arc knob (copper) + label; **SEED** numeric stepper (mono text, ‹ › arrows).

## Controls inventory (visible at once)

- 7 arc knobs (3 band + 3 output + 1 entropy header)
...
- All tokens inherited from Chaosverb (see v1-style-guide.md). No new surface colors.
