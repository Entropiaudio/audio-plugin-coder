...
- Bell (height = LIFT): fill band-hue @18%, stroke band-hue @85% 1.5px, glow on selected (shadow blur 8 band-hue); apex handle 8px dot, white core.
- Pan trace: 2px band-hue line sliding L/R within bell width at bell mid-height.
- Step editor: bars fill band-hue @35%, active step bright, center line `--border-bright`; ratchet sub-steps = narrower bars + tick markers (band-hue @93%) on top edge; preset row = compact select (`--bg-elevated`) + SAVE ghost button.
- Scope: `--bg-elevated` panel, center line `--border-bright`, L/R rails `--border-divider`, waveform = band hue @87% 1.5px, white live dot. Dimmed (27%) when band bypassed.
- Quick-menu: floating 118px card, `rgba(36,32,29,0.92)` + blur, border = band hue @53%, 7px radius; grip bar (two 22×2px `--border-bright` lines, grab cursor) + two 17px mini-slider rows (label 7px / value 7.5px mono).
- Global speed (header): label `--accent` 8.5px + glow; value `--accent-bright` 15px semibold + glow; arrows `--accent` outlined.
